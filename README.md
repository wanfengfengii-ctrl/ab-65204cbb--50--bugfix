...
- 容量判定以**精确有理数**（`fractions.Fraction`）进行，不设有效数字上限：
  输入整数部分再长（如 60 位）也不会因舍入改变 `V+x` 与 `K` 的关系；
  返回的十进制数值按 `decimal` 50 位有效数字展示，输入超过 50 位时自适应扩位
...
- 以**精确有理数**反解最小排出量（比较不设有效数字上限），使排出后补加的终态恰好顶到容量上限：
...
- 算法**不逐刻度扫描**：以**精确有理数**（无有效数字上限）从容量上界 `K-V`、
...
```bash
cd api && python -m pytest tests/ -v     # 后端：86 例
