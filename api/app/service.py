...
def _endpoint_deviation(
    n: int, Q: Decimal, V: Decimal, C: Decimal, T: Decimal, low: Decimal, high: Decimal
) -> tuple[Decimal, Decimal, Decimal]:
    """刻度 n（剂量 nQ）在两个端点糖度下的终态糖度与最坏偏差。"""
    x = Q * n
...
def robust_plan(req: RobustPlanRequest) -> dict:
    ...
    with localcontext() as ctx:
        ...
        # 容量边界刻度：满足 V + nQ ≤ K 的最大正整数 n
        n_max = int((capacity_dose / Q).to_integral_value(rounding=ROUND_FLOOR))
        if n_max < 1:
            ...
            for point in breakpoints:
                nearest = point / Q
                lower = int(nearest.to_integral_value(rounding=ROUND_FLOOR))
                for n in (lower, lower + 1):
                    if 1 <= n <= n_max:
                        candidates.add(n)
            best_n = best_low = best_high = best_deviation = None
            for n in sorted(candidates):
                final_low, final_high, deviation = _endpoint_deviation(
                    n, Q, V, C, T, low_syrup, high_syrup
                )
                # 按 n 升序评估，严格更小才替换：同值保留较小 n
                if best_deviation is None or deviation < best_deviation:
                    best_n, best_low, best_high, best_deviation = (
                        n,
                        final_low,
                        final_high,
                        deviation,
            ...
            if best_deviation <= E:
                ...
                plan = {
                    ...
                    "finalSugarHigh": None,
                    "worstDeviation": None,
                    "toleranceGap": _plain(best_deviation - E),
                }
